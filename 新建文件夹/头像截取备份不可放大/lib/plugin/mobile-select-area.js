	...
	MobileSelectArea.prototype = {
		...
		bindEvent: function() {
			...
			this.trigger[_this.settings.eventName](function(e) {
				...
				$('.ui-dialog-mask,.ui-dialog-action').bind("touchmove",function(e){  //modify zhangw  防止遮罩层滑动
               		 e.preventDefault();
   				});
				return false;
			});
		...
		f: function(data) {
			...
			//modify by zhangw
			if(_this.index == 0){
				$(child[_this.index]).find('dd').eq(0).text('省份');
			}else if(_this.index == 1){
				$(child[_this.index]).find('dd').eq(0).text('城市');
			}else if(_this.index == 2){
				$(child[_this.index]).find('dd').eq(0).text('区县');
			}
