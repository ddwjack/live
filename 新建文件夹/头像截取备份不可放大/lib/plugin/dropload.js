...
;(function($){
    ...
    function fnTouchmove(e, me){
        ...
        if(me.opts.loadUpFn != '' && me._scrollTop <= 0 && me.direction == 'down'){
            ...
            me.$domUp.css({'height': me._offsetY-150});
        }
        ...
        if(me.insertDOM){
            ...
            if(_absMoveY > me.opts.distance){
               ...
               // me.$domResult.html('').append(me.domLoad);
			    me.$domResult.html('');   //测试代码 zhangw
				setTimeout(function(){
					 me.$domResult.html(me.domLoad);
				}, 10);
                fnCallback(me);
            }else{
				if(me.$domResult && me.$domResult.length > 0){
					me.$domResult.css({'height':'0'}).on('webkitTransitionEnd',function(){
						//me.loading = false; //测试代码 zhangw
						me.insertDOM = false;
						$(this).remove();
					});
				}
    ...
    MyDropLoad.prototype.resetload = function(){
        ...
        if(me.$domResult && me.$domResult.length > 0){
			//setTimeout(function(){
				me.$domResult.html(' <div class="dropload-load">刷新成功</div>');
				////console.log(me.$domResult.html());
			//},30)
			////console.log(me.$domResult);
			//alert(me.$domResult.html());
			setTimeout(function(){
				me.$domResult.css({'height':'0'}).on('webkitTransitionEnd',function(){
					me.loading = false;
					me.insertDOM = false;
		...
		me.loading = false; //测试代码 zhangw
		me.insertDOM = false; //测试代码 zhangw
    };
