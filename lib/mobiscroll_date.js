...
(function ($) {
        ...
        that.setValue = function (sc, fill, time, temp) {
            ...
            if (fill) {
                if (input) {
                    ...
                    if ($('#USER_RI').attr('data-l') == '2') {
                        // alert(1)
                        var arrNm = {'birthday':v}
                        Global.usNoetu(arrNm,'2')
                    }else{
                        if ($('#inpOntin').attr('data-l') =='ON') {
                            var ONs = $('#inpOntin').val() // 开始时间
                            var OFFs = $('#inpOfftin').val() // 结束时间
                            if (ONs > OFFs) {
                                $.alertMsg('開始日期不能大於結束日期')
                                $('#inpOntin').val(OFFs).attr('placeholder',OFFs)
                            }else{
                                moneyObj.updatePlay('Inquire',ONs,OFFs)
                            }
                        }else if($('#inpOfftin').attr('data-l') =='OFF'){ // 此处未走
                            var ONs = $('#inpOntin').val()
                            var OFFs = $('#inpOfftin').val()
                            if (ONs < OFFs) {
                                $.alertMsg('結束日期不能小於開始日期')
                            }else{
                                moneyObj.updatePlay('Inquire',ONs,OFFs)
                            }
                        }
    ...
    }).bind('mousedown', function (e) { // Prevent standard behaviour on body click
        if (tap) {
            // alert(1105)
            e.stopPropagation();
            e.preventDefault();
