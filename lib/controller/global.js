...
//      //console.log('Global.GC销毁页面', tempArr); overlayerOk
    ...
    apkName:function(obj){
        ...
        var html2 = '(协议版本v2.1.8)'
        // apkNam = '蝌蚪主播'
        Img = 'images/sjb.png'
        $(".p_Img").attr('src',Img); 
        $('.s_nam').html(html)
        $('#video_js').html(html2)
        $('.spanNameUser').html('青蛙社区')
        // plusObj(10)
        // Global.chshareObj('anchor') // anchor/user
        // goToLogin()
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if (!res.err) {
                    ...
                    sharePicObj(res.info.message)
                    ConfigObj.share_url = res.info.url //分享地址 
        ...
        $.ajax({
            ...
            success:function(res){
                if (res.err == undefined) {
                    ...
                    ConfigObj.rules = res.info.basic.rules  // 0 是可提现 1 是不可提现
                    ConfigObj.profi = res.info.basic.promote_profit
...
//      //console.log(obj);
    ...
    encrypt:function(data){ //正式的加密
        var postData = JSON.stringify(data);
        var srcs = CryptoJS.enc.Utf8.parse(postData);
        var encrypted = CryptoJS.AES.encrypt(srcs, CryptoJS.enc.Utf8.parse("AUCj067VsOWgmyFR"), { iv: CryptoJS.enc.Utf8.parse("MNZuDexL4UHGCfyW"),mode:CryptoJS.mode.CBC,padding: CryptoJS.pad.Pkcs7});
        var postDataHexStr = CryptoJS.enc.Hex.parse(encrypted.ciphertext.toString().toUpperCase());
        var postDataEnd = CryptoJS.enc.Base64.stringify(postDataHexStr);
        var str = b64EncodeUnicode(postDataEnd);
        return str;
    ...
    crypt:function(data){
        // console.log(data)
        var data = b64DecodeUnicode(data);
        var decrypt = CryptoJS.AES.decrypt(data, CryptoJS.enc.Utf8.parse("AUCj067VsOWgmyFR"), { iv: CryptoJS.enc.Utf8.parse("MNZuDexL4UHGCfyW"),mode:CryptoJS.mode.CBC,padding: CryptoJS.pad.Pkcs7});
        var decryptedStr = decrypt.toString(CryptoJS.enc.Utf8);
        return decryptedStr.toString();
    },
}
...
    timer = null;
        ...
        $.ajax({
            ...
            success:function(res){
                if (res.err == undefined) {
                    ...
                    }else if(res.info.basic.auth_status == '1'){
                        $('.gozbrz').hide()
    ...
    function plusObj(num){
        // alert(num + '加')
        // alert(localStorage.getItem("Imtextlive") + '在2317') sp_Imnums
        var Imtext = ($('#i_ImNums').html()*1) + (num*1)
        if (Imtext <= '0') {
            $('#sp_Imnums').hide()
            ...
            // $('#i_ImNums').html('99')
            $('#sp_Imnums').html('<i id="i_ImNums">99</i>'+'+')
            $('#sp_Imnums').show()
            localStorage.setItem("Imtextlive", '99');
            $('#liveImNum').html('99+')
        }else{
            // console.log(2318)
            $('#i_ImNums').html(Imtext)
            // $('#sp_Imnums').html('<i id="i_ImNums">'+ Imtext +'</i>')
            // alert(Imtext + '在2334')
    ...
    function lessObj(num){
        var Imtext = ($('#i_ImNums').html()*1) - (num*1)
        if (Imtext <= '0') {
            $('#sp_Imnums').css('display','none')
            localStorage.setItem("Imtextlive", '0');
            $('#liveImNum').html('0')
            $('#i_ImNums').html('0')
        }else if (Imtext >= '99') {
            $('#sp_Imnums').html('<i id="i_ImNums">99</i>'+'+')
            $('#sp_Imnums').css('display','inline-block')
            localStorage.setItem("Imtextlive", '99');
            $('#liveImNum').html('99+')
        }else{
            $('#i_ImNums').html(Imtext)
    ...
    // ios input輸入框  測試
    function inpblur(typ){
        var interval;
        // console.log(typ)
        typ.onfocus = function(){
            interval = setInterval(function(){
                typ.scrollIntoView(true) 
                // typ.scrollIntoViewIfNeeded()
                // console.log(typ.scrollIntoView(true))
            }, 1000);
    ...
    function getUnReads(){
        // console.log(2406)
         if (ConfigObj.platForm == 'android') {
            var liveImlist = android_obj.getUnRead()
            if (liveImlist == '0') {
                $('#sp_Imnums').hide()
                localStorage.setItem("Imtextlive", '0');
                $('#i_ImNums').html('0')
                $('#liveImNum').html('0')
            }else if(liveImlist >= '99'){
                $('#sp_Imnums').show()
                localStorage.setItem("Imtextlive", '99');
                // $('#i_ImNums').html('99+')
                $('#sp_Imnums').html('<i id="i_ImNums">99</i>'+'+')
                ...
                $('#sp_Imnums').show()
                localStorage.setItem("Imtextlive", liveImlist);
                $('#i_ImNums').html(liveImlist)
                $('#liveImNum').html(liveImlist)
            } 
        }else if(ConfigObj.platForm == 'ios'){
            var liveImlists = ios_obj.getUnRead()
            if (liveImlists == '0') {
                $('#sp_Imnums').hide()
                localStorage.setItem("Imtextlive", liveImlists);
                $('#i_ImNums').html('0')
                $('#liveImNum').html(liveImlists)
            }else if(liveImlists >= '99'){
                $('#sp_Imnums').show()
                localStorage.setItem("Imtextlive", '99');
                $('#sp_Imnums').html('<i id="i_ImNums">99</i>'+'+')
                // $('#i_ImNums').html('99+')
